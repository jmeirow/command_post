...
total = 0.00
cnt = 0
TestXXXPerson.hourly_rate_eq(40.00).sort{|x,y| [x.last_name, x.first_name] <=> [y.last_name, y.first_name]}.each do | person |
  cnt += 1
  last_name = "%20s" % person.last_name
  first_name = "%20s" % person.first_name
  hourly_rate = "%.2f" % person.hourly_rate
  ssn = person.ssn
  total += person.hourly_rate
  puts "#{last_name}  #{first_name}    #{ssn}    $#{hourly_rate}"
end


print_total = "%.2f" % total 
puts "This group of #{cnt} people earn $#{print_total} per hour in aggregate."
